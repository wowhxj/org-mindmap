...
* What is it
A tool to draw mind maps in an Org Mode blocks, optimized for fast input and editing. Not perfect visually, but pretty enough to be attractive. The map is just text, no rendering is required. And since a mind map is just a list drawn in a fancy way, you can convert a list into a map and the other way around.

Just =C-c C-, m= anywhere in an Org buffer and you can start writing. Move the nodes around as your map grows. Focus on the content, let the engine handle layout and coloring. 
...
* Visuals
The package does its best to let you focus on the map contents and structure, by providing reasonably good visuals without ability to tweak the minuscine detains by hand:
...
- it paints subtrees into different colors, so that you can visually discert which node belongs to which subtree — but you can't assign individual colors to nodes.
...
** Layouts
No formatting inside nodes text is supported yet. You can use Org text modifiers (bold, italics, code etc) but beware the 
...
- the palette (via =org-mindmap-palette-fn=) and blending proportions (=org-mindmap-paint-tinge-fg= and =org-mindmap-paint-tinge-bg=);
...
** Right-to-Left
Left-to-right text may work, but it was not tested. If you have bugs, please report.
